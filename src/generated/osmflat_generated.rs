...
pub mod osm {
        ...
        fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
            f.debug_struct("Header")
                ...
                .field(
                    ...
                    &self.replication_sequence_number(),
                )
    ...
    impl std::cmp::PartialEq for Header {
        ...
        fn eq(&self, other: &Self) -> bool {
            self.coord_scale() == other.coord_scale()
                ...
                && self.replication_timestamp() == other.replication_timestamp()
                && self.replication_sequence_number() == other.replication_sequence_number()
                && self.replication_base_url_idx() == other.replication_base_url_idx()
        }
    }
        ...
        pub fn open(
            ...
            let header = {
                ...
                check(
                    ...
                    |_| 0,
                    max_size,
                    resource.and_then(|x| super::osm::Header::from_bytes_slice(x)),
                )?
            };
            let nodes = {
                use flatdata::check_resource as check;
                ...
                check(
                    ...
                    |r| r.len(),
                    max_size,
                    resource.and_then(|x| <&[super::osm::Node]>::from_bytes(x)),
                )?
            };
            let hilbert_node_pairs = {
                ...
                let resource = extend(storage.read(
                    "hilbert_node_pairs",
                    schema::osm::resources::HILBERT_NODE_PAIRS,
                ));
                check(
                    "hilbert_node_pairs",
                    |r| r.len(),
                    max_size,
                    resource.and_then(|x| <&[super::osm::HilbertNodePair]>::from_bytes(x)),
                )?
            };
            let ways = {
                use flatdata::check_resource as check;
            ...
            let tags = {
                ...
                check(
                    ...
                    |r| r.len(),
                    max_size,
                    resource.and_then(|x| <&[super::osm::Tag]>::from_bytes(x)),
                )?
            };
            let tags_index = {
                use flatdata::check_resource as check;
                let max_size = Some(1099511627776);
                let resource =
                    extend(storage.read("tags_index", schema::osm::resources::TAGS_INDEX));
                check(
                    "tags_index",
                    |r| r.len(),
                    max_size,
                    resource.and_then(|x| <&[super::osm::TagIndex]>::from_bytes(x)),
                )?
            };
            let nodes_index = {
                use flatdata::check_resource as check;
                let max_size = Some(1099511627776);
                let resource =
                    extend(storage.read("nodes_index", schema::osm::resources::NODES_INDEX));
                check(
                    "nodes_index",
                    |r| r.len(),
                    max_size,
                    resource.and_then(|x| <&[super::osm::NodeIndex]>::from_bytes(x)),
                )?
            };
            let stringtable = {
                use flatdata::check_resource as check;
                let max_size = Some(1099511627776);
                let resource =
                    extend(storage.read("stringtable", schema::osm::resources::STRINGTABLE));
                check(
                    "stringtable",
                    |r| r.len(),
                    max_size,
    ...
    impl OsmBuilder {
        ...
        pub fn set_hilbert_node_pairs(
            &self,
            vector: &[super::osm::HilbertNodePair],
        ) -> ::std::io::Result<()> {
            use flatdata::SliceExt;
            self.storage.write(
                "hilbert_node_pairs",
        ...
        /// [`ExternalVector::close`]: flatdata/struct.ExternalVector.html#method.close
        #[inline]
        pub fn start_hilbert_node_pairs(
            &self,
        ) -> ::std::io::Result<flatdata::ExternalVector<super::osm::HilbertNodePair>> {
            flatdata::create_external_vector(
                &*self.storage,
                "hilbert_node_pairs",
                schema::osm::resources::HILBERT_NODE_PAIRS,
